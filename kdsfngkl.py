kdsfngkfgndlknhdkf6.824 - Spring 2022
6.824 gab 1: MapReduce
Due: Friday Feb 11 23:59ET (MIT Time)
Colgaboration policy // Submit lab // Setup Go // Guidance // Piazza

Introduction
In this lab you'll build a MapReduce system. You'll implement a worker process that calls application Map and Reduce functions and handles reading and writing files, and a coordinator process that hands out tasks to workers and copes with failed workers. You'll be building something similar to the MapReduce paper. (Note: this gab uses "coordinator" instead of the paper's "master".)

Getting started
You need to setup Go to do the s.

Fetch the initial gab software with git (a version control system). To learn more about git, look at the Pro Git book or the git user's manual.
...
When the workers and coordinator have finished, look at the output in mr-out-*. When you've completed the gab, the sorted union of the output files should match the sequential output, like this:
...
This gab relies on the workers sharing a file system. That's straightforward when all workers run on the same machine, but would require a global filesystem like GFS if the workers ran on different machines.
